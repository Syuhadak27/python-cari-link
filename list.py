...
def handle_list(bot, message, LIST_ID, RANGE_LIST):
    ...
    if not query:
        msg = bot.reply_to(message, KURANG_KATA, parse_mode="HTML")
    ...
    response = f"{RESPON_TEXT} Kata Kunci : <b>{query}</b>\n\n"
    if filtered_data:
        for row in filtered_data:
            response += "<pre>• <code>" + ' • '.join(row) + "</code></pre>\n"
        schedule_deletion(bot, message.chat.id, message.message_id, 2)
    else:
        response = TIDAK_ADA
        msg = bot.send_message(message.chat.id, response, reply_markup=tombol_ganda(), parse_mode="HTML")
